...
We compare the performance of Utf8Validator, either Python pure code running on CPython or PyPy (which is the attached `utf8balidator.py`) or a Cython port of that code (the code is really a straight forward rewrite .. no algo change or such):

   https://github.com/methane/wsaccel/blob/master/wsaccel/utf8validator.pyx
...
This shows that currently for this workload CPython/wsaccel is fastest. And this results matches the one from full-scale WebSocket testing - look at test 9.1.6 the first 3 columns here:
